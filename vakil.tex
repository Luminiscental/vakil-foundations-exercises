...
\begin{enumerate}[label=\textbf{1.3.\Alph*.}]
	\item If $A,B$ are initial objects, then since there is always at least
	      ...
	      isomorphisms in $\mathscr{C}^\opp$, final objects are also
	      uniquely isomorphic.
...
\begin{enumerate}[label=\textbf{6.5.\Alph*.}]
	...
	\item If $V\subseteq Y$ is the image of the rational map, then we have
	      $\Gamma(V,\O_Y)\to K(X)$, and this factors through a map $K(Y)\to K(X)$
	      since $K(Y)$ is the fraction field of $\Gamma(V,\O_Y)$.
