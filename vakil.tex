...
\begin{enumerate}[label=\textbf{7.3.\Alph*.}]
	...
	\item It suffices to prove the case where $X=\Spec A$, since otherwise $X$
	      is covered by such sets, and hence is a discrete set---possibly
	      infinite, but otherwise as described. To be finite over $k$ it must be
	      a finite set, since $\Gamma(X)$ is the direct product of all the
	      stalks.
	...
	\item We show that being locally of finite presentation is affine-local on
	      ...
	      \begin{itemize}
		      ...
		      \item If each $A_{f_i}$ is finitely presented, then $A$ is
		            ...
		            \end{align*}
		            If $S=B[x_1,\ldots,x_n]/(\{r^{(j)}_k\}_{jk})$, then the
		            surjection $S\to A$ becomes an isomorphism when localizing at
		            each $s_i\in S$. Since $(f_1,\ldots,f_r)=1$ in $A$, we can add
		            a relation ensuring $(s_1,\ldots,s_n)=1$ in $S$. Then $S\to A$
		            is an isomorphism, since it is an isomorphism locally, so $A$
