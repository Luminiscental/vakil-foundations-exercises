...
\renewcommand{\set}{{\mathrm{set}}} % set label
...
% TODO: spectral sequences proof; section 1.7.7

% TODO: fibered products by representability ('e)

% TODO: \Gal(\closure\Q/\Q) exercise 9.2.E ('f)
...
\begin{enumerate}[label=\textbf{8.4.\Alph*.}]
	...
	\item Since $S_n$ is generated by transpositions of the form
	      $i\leftrightarrow i-1$, it suffices to show that the sequence obtained
	      by swapping $x_i$ and $x_{i-1}$ remains regular for any $i$. But
	      $x_1,\ldots,x_{i-2}$ is regular on $M$, and $x_{i-1},x_i$ is regular
	      on $M/(x_1,\ldots,x_{i-2})$---which remains finitely generated---so by
	      the $r=2$ case $x_i,x_{i-1}$ is regular on $M/(x_1,\ldots,x_{i-2})$.
	      Since $x_{i+1},\ldots,x_r$ is regular on
	      $M/(x_1,\ldots,x_i)=M/(x_1,\ldots,x_{i-2},x_i,x_{i-1})$, we get that
	      $x_1,\ldots,x_{i-2},x_i,x_{i-1},x_{i+1},\ldots,x_r$ is regular on $M$.
	      The Noetherian hypothesis is used to ensure that $(0:(y))$ is finitely
	      generated in the proof of the $r=2$ case.

	\item Since the condition is local, we can restrict to an open
	      neighbourhood of $p$, and hence assume $\pi$ is a closed embedding,
	      and that $Y$ is affine. Say $Y=\Spec B$, $X=\Spec B/I$, $p=[\p]$. By
	      assumption there is a regular sequence $f_1,\ldots,f_r$ in $B_\p$
	      generating $I_\p$. We may assume $f_1,\ldots,f_r\in B$, since unit
	      multiples do not affect regularity. If $\psi_i$ denotes the
	      multiplication map of $f_i$ on $B/(f_1,\ldots,f_{i-1})$, then
	      $(\ker\psi_i)_\p=0$ by exactness of localization. By the following
	      result, we get $(\ker\psi_i)_g=0$ for each $i$ for some $g\notin\p$,
	      by taking the product of finitely many such $g$. By a further
	      application of the result we may assume $(f_1,\ldots,f_r)_g=I_g$,
	      since $(f_1,\ldots,f_r)_\p=I_\p$. Hence $f_1,\ldots,f_r$ is a regular
	      sequence in $B_g$ generating $I_g$. (Note that $I_g\ne(1)$ since
	      $I_\p\ne(1)$.) It follows that $\pi$ is a regular embedding at any
	      point of $D(g)\cap X$ by 8.4.D, since $I_\q=(1)$ iff $[\q]\notin X$.

	      \textbf{Claim:} If $\p\subseteq A$ is a prime in a Noetherian ring,
	      and $I,J\subseteq A$ are ideals with $I_\p=J_\p$, then there is some
	      $g\in A\setminus\p$ with $I_g=J_g$.

	      \textbf{Proof:} Since $I=J$ iff $I\cap J=J$ and $I\cap J=I$, and
	      localization commutes with finite intersections, we may assume
	      $I\subseteq J$. Then $M=J/I$ is a finitely-generated $A$-module with
	      $M_\p=0$, so $gM=0$ for some $g\in A\setminus\p$. Hence $J_g=I_g$ as
	      required.

	\item If $X\hookrightarrow Y$ is a regular embedding of codimension 1,
	      then from the solution to 8.4.G we have affine open sets covering $X$
	      on which $\I_{X/Y}$ is generated by a non-zerodivisor, and $\I_{X/Y}$
	      is generated by 1 on affine open sets in $Y\setminus X$, so
	      $X\hookrightarrow Y$ is an effective Cartier divisor.

	      Conversely if $X\hookrightarrow Y$ is an effective Cartier divisor,
	      then $Y$ is covered by affine open sets on which $\I_{X/Y}$ is
	      generated by a non-zerodivisor. The localization of a non-zerodivisor
	      is a non-zerodivisor since localization is exact, and so the stalks of
	      $\I_{X/Y}$ are each generated by a non-zerodivisor. Moreover the
	      stalks are non-trivial at points of $X$, so $X\hookrightarrow Y$ is a
	      regular embedding of codimension 1.
\end{enumerate}

\chapter{Fibered products of schemes, and base change}

\begin{enumerate}[label=\textbf{9.1.\Alph*.}]
	\item Note that the topological space $X\amalg Y$ is the coproduct of $X$
	      and $Y$ as topological spaces, with the inclusions $i_X:X\to X\amalg Y$,
	      $i_Y:Y\to X\amalg Y$. Also the sheaf
	      $\O_{X\amalg Y}(U)=((i_X)_*\O_X)(U)\times((i_Y)_*\O_Y)(U)$ is the product of
	      $(i_X)_*\O_X$ and $(i_Y)_*\O_Y$ as sheaves on $X\amalg Y$. These
	      universal properties combine to make $(X\amalg Y,\O_{X\amalg Y})$ the
	      coproduct of $X$ and $Y$ as ringed spaces. It is a scheme, since $X$ and
	      $Y$ give an open cover, and are schemes.

	\item Having morphisms of $C$-schemes $X\to\Spec A$, $X\to\Spec B$
	      corresponds to having maps of $C$-algebras $A\to\Gamma(X)$,
	      $B\to\Gamma(X)$. This in turn corresponds to having a map of
	      $C$-algebras $A\otimes_CB\to\Gamma(X)$, and hence to having a morphism
	      $X\to\Spec(A\otimes_CB)$.

	\item Given a morphism $X\to Y$, we get a natural transformation
	      $h_X\to h_Y$ by composition. Conversely, if $\eta:h_X\to h_Y$ is a
	      natural transformation then we have a morphism $\eta_X(\id_X):X\to Y$.
	      This sets up a bijection, since
	      $\eta_Z(f)=\eta_Z(\id_X\circ f)=\eta_X(\id_X)\circ f$ by naturality.
...
\begin{enumerate}[label=\textbf{9.2.\Alph*.}]
	\item We have natural bijections
	      \begin{align*}
		      \Mor_{\cat{Alg}_B}(A\otimes_BB[t],C)
		       & \cong \Mor_{\cat{Alg}_B}(A,C)\times\Mor_{\cat{Alg}_B}(B[t],C) \\
		       & \cong \Mor_{\cat{Alg}_B}(A,C)\times|C|                        \\
		       & \cong \Mor_{\cat{Alg}_B}(A[t],C)
	      \end{align*}
	      so this follows from the Yoneda lemma.

	\item We have the exact sequence $I\to B\to B/I\to0$, and since the tensor
	      product is right exact this gives an exact sequence
	      $A\otimes_BI\to A\otimes_BB\to A\otimes_B(B/I)\to0$. Now using $\phi$
	      and multiplication on $A$, we get vertical maps as follows:
	      \begin{equation*}
		      \begin{tikzcd}
			      &A\otimes_BI \ar[r,"f"] \ar[d,"m_I"]
			      &A\otimes_BB \ar[r] \ar[d,"m_B"]
			      &A\otimes_B(B/I) \ar[r] &0 \\
			      &I^e \ar[r,hook,"\iota"] &A & &
		      \end{tikzcd}
	      \end{equation*}
	      Now $m_I$ and $m_B$ are surjective, so $\im\iota=m_B(\im f)$. Moreover
	      $m_B$ is an isomorphism, so we get a sequence
	      $I^e\to A\to A\otimes_B(B/I)\to0$ which is also exact.

	\item
	      \begin{enumerate}[label=(\alph*)]
		      \item Locally, the fibered product is given by
		            \begin{equation*}
			            \Spec B/I\times_{\Spec B}\Spec B/J
			            = \Spec\bigl((B/I)\otimes_B(B/J)\bigr),
		            \end{equation*}
		            and from 9.2.B we have
		            \begin{equation*}
			            (B/I)\otimes_B(B/J)
			            \cong (B/I)/J^e
			            = (B/I)/\bigl((I+J)/I\bigr)
		      ...
		      \item Suppose $X'\hookrightarrow U\hookrightarrow X$ is a locally
		            closed embedding, and we have morphisms $X\to Z$, $Y\to Z$.
		            Since open and closed embeddings are preserved by base change we
		            get
		            \begin{equation*}
			            \begin{tikzcd}
				            &W'' \ar[r] \ar[d,hook,"\rm{closed}"]
				            &X' \ar[d,hook,"\rm{closed}"] \\
				            &W' \ar[r] \ar[d,hook,"\rm{open}"]
				            &U \ar[d,hook,"\rm{open}"] \\
				            &W \ar[r] \ar[d] &X \ar[d] \\
				            &Y \ar[r] &Z
			            \end{tikzcd}
		            \end{equation*}
		            where $W=X\times_ZY$, $W'=W\times_XU$, $W''=W'\times_UX'$, so
		            each inner square is Cartesian. Then the outer square is
		            Cartesian, so $X'\times_ZY$ has a canonical locally closed
		            embedding into $X\times_ZY$.

		      \item It suffices to consider the case $n=2$, since
		            $X_1\times_Z(X_2\times_Z(\cdots\times_ZX_n))$ satisfies the same
		            universal property as the all-at-once fibered product over $Z$,
		            and a composite of locally closed embeddings is a locally closed
		            embedding. In this case we get
		            \begin{equation*}
			            \begin{tikzcd}
				            &X_1\times_ZX_2 \ar[r,hook] \ar[d,hook] &X_2 \ar[d,hook] \\
				            &X_1 \ar[r,hook] &Z
			            \end{tikzcd}
		            \end{equation*}
		            where the structure morphisms from $X_1\times_ZX_2$ are locally
		            closed embeddings by (b). Since a composite of locally closed
		            embeddings is a locally closed embedding, it follows that
		            $X_1\times_ZX_2\to Z$ is a locally closed embedding.
	      \end{enumerate}

	\item Since $X\times_AY$ is covered by affine open sets of the form
	      $\Spec(B\otimes_AC)$ where $\Spec B\subseteq X$,
	      $\Spec C\subseteq Y$ are open, it suffices to show that
	      $B\otimes_AC$ is finitely generated over $A$ whenever $B$ and
	      $C$ are, using 7.3.O. But if $\{b_i\}\subseteq B$ and
	      $\{c_j\}\subseteq C$ are finite generating sets, then
	      $\{b_i\otimes 1\}\cup\{1\otimes c_j\}\subseteq B\otimes_AC$ is a
	      finite generating set.

	      Moreover if $X$ and $Y$ are quasicompact, then we can take
	      finitely many such $\Spec B$ and $\Spec C$ while still covering
	      $X$ and $Y$, which gives finitely many $\Spec(B\otimes_AC)$
	      covering $X\times_AY$, so $X\times_AY$ is quasicompact.

	\item $\star$ skipped.

	\item We have a map $A\otimes_B(S^{-1}B)\to\phi(S)^{-1}A$ given by
	      $a\otimes(b/s)\mapsto a\phi(b)/\phi(s)$, since this is $B$-bilinear
	      in $a$ and $b/s$. Conversely, the map $A\to A\otimes_B(S^{-1}B)$
	      where $a\mapsto a\otimes1$ factors through $\phi(S)^{-1}A$, since
	      $\phi(s)\otimes1$ has inverse $1\otimes(1/s)$. Hence we get
	      $\phi(S)^{-1}A\cong A\otimes_B(S^{-1}B)$.

	\item If a morphism of ringed spaces is a topological embedding, and is
	      surjective on stalks, then it is a monomorphism; suppose $i:X\to Y$
	      is such a morphism. We get that
	      \begin{itemize}
		      \item The underlying map $X\to Y$ is a monomorphism of
		            topological spaces, since it is a topological embedding
		            (hence injective on points).

		      \item The pullback $\O_Y\to i_*\O_X$ is an epimorphism of
		            sheaves on $Y$, since it is surjective on stalks.

		      \item The pushforward $i_*$ is a faithful functor from sheaves
		            on $X$ to sheaves on $Y$: if $i_*\phi=i_*\psi$ for
		            $\phi,\psi:\scrF\to\scrG$, then
		            $\phi(i^{-1}(W))=i_*\phi(W)=i_*\psi(W)=\psi(i^{-1}(W))$ for
		            any open $W\subseteq Y$, and every open subset of $X$ is of
		            this form since $i$ is a topological embedding.
	      \end{itemize}
	      Hence if $f,g:Z\to X$ are morphisms with $i\circ f=i\circ g$, then
	      the underlying continuous maps of $f$ and $g$ coincide, and the two
	      maps $i_*\O_X\to i_*(f_*\O_Z)=i_*(g_*\O_Z)$ coincide, so that the
	      pullbacks $\O_X\to f_*\O_Z=g_*\O_Z$ also coincide.

	      Now open embeddings, closed embeddings, and localizations are all
	      topological embeddings. Moreover they are surjective on stalks (for
	      open embeddings and localizations the stalk maps are isomorphisms,
	      and for closed embeddings this follows from exactness of
	      localization). Hence they are all monomorphisms of schemes.

	\item We have $\A^n_A\cong\A^n_\Z\times_\Z\Spec A$ since
	      $\Z[x_1,\ldots,x_n]\otimes_\Z A\cong A[x_1,\ldots,x_n]$ by 9.2.A and
	      induction. For $\P^n_A$, note that taking the isomorphism
	      $\Z[x_0/x_i,\ldots,x_n/x_i,x_i/x_j]\cong\Z[x_0/x_j,\ldots,x_n/x_j,x_j/x_i]$
	      and tensoring with $A$ gives the isomorphism
	      $A[x_0/x_i,\ldots,x_n/x_i,x_i/x_j]\cong A[x_0/x_j,\ldots,x_n/x_j,x_j/x_i]$
	      used to construct $\P^n_A$, and hence
	      $\P^n_\Z\times_\Z\Spec A\cong\P^n_A$.

	      % ???
	\item Since $X\times_k\Spec\ell\to X$ is surjective, any $p\in X$ is the
	      image of some $q\in X\times_k\Spec\ell$. Then $\pi(p)$ and $\rho(p)$
	      are both the image of $\pi_\ell(q)=\rho_\ell(q)$ under
	      $Y\times_k\Spec\ell\to Y$, so $\pi(p)=\rho(p)$.

	      We can also reduce to the case where $Y$ is affine, since if $\pi$
	      and $\rho$ agree on the preimage of any affine open subscheme of
	      $Y$, then they agree on all of $X$. (The extension of a
	      restricted map $\pi^{-1}(\Spec B)\to\Spec B\subseteq Y$ by $\ell/k$
	      is the same as the restriction of $\pi_\ell$ to
	      $\pi_\ell^{-1}(\Spec B\times_k\Spec\ell)$, where
	      $\Spec B\times_k\Spec\ell$ is an affine open subscheme of
	      $Y\times_k\Spec\ell$ by the construction of the fibered product.)

	      Moreover we can then reduce to the case where $X$ is affine, since
	      if $\pi$ and $\rho$ agree when restricted to any affine open
	      subscheme of $X$, then they agree on all of $X$. (The extension of a
	      restricted map $\pi|_{\Spec A}$ by $\ell/k$ is the same as the
	      restriction of $\pi_\ell$ to $\Spec A\times_k\Spec\ell$, where
	      $\Spec A\times_k\Spec\ell$ is an affine open subscheme of
	      $X\times_k\Spec\ell$ by the construction of the fibered product.)

	      So we are reduced to showing that two maps of $k$-algebras $B\to A$
	      agree whenever the tensored maps $B\otimes_k\ell\to A\otimes_k\ell$
	      agree. We have the following square
	      \begin{equation*}
		      \begin{tikzcd}
			      &A \ar[r] \ar[d] &B \ar[d] \\
			      &A\otimes_k\ell \ar[r] &B\otimes_k\ell
		      \end{tikzcd}
	      \end{equation*}
	      where the vertical maps are injections ($k\to\ell$ is an injection,
	      and tensoring with $A$ or $B$ preserves this since vector spaces are
	      flat). Then $A\to B$ is determined by the composite
	      $A\to B\to B\otimes_k\ell$ since $B\to B\otimes_k\ell$ is an
	      injection, and this equals the composite
	      $A\to A\otimes_k\ell\to B\otimes_k\ell$. Hence $A\to B$ is
	      determined by $A\otimes_k\ell\to B\otimes_k\ell$ as required.

	\item We have $\ell\cong k^{\oplus I}$ as $k$-modules for some
	      $I\ne\emptyset$, and want to show that a $k$-algebra map $f:A\to B$
	      is surjective iff the induced map
	      \begin{equation*}
		      A\otimes_k\ell\cong A^{\oplus I}
		      \to B^{\oplus I}\cong B\otimes_k\ell
	      \end{equation*}
	      is surjective. The forward implication is clear (it even follows
	      directly from right-exactness of the tensor product). For the
	      converse, pick $i_0\in I$ and consider
	      $\sum_{i\in I}\delta_{i,i_0}b\in B^{\oplus I}$. This is the image of
	      some $\sum_{i\in I}a_i\in A^{\oplus I}$, and then $b=f(a_{i_0})$.

	\item The residue field of a closed point is a minimal mutual extension
	      of $\Q(t)$ and $\C$. If the image of $t$ is contained in the image
	      of $\C$, then the closed point comes from the kernel of the map
	      $\Q(t)\otimes_\Q\C$ which fixes $\C$ and sends $t$ to this
	      distinguished transcendental complex number. Distinct choices give
	      distinct maximal ideals, e.g. based on which value of the form
	      $t\otimes1-1\otimes z$ is present.

	      The only other possible residue field would be $\C(t)$. But
	      $\Q(t)\otimes_\Q\C\to\C(t)$ is not surjective, since
	      $\Q(t)\otimes_\Q\C$ only has fractions of the form $P(t)/Q(t)$ with
	      $P(t)\in\C[t]$, $Q(t)\in\Q[t]$, missing out rational functions such
	      as $1/(t-i)$.
\end{enumerate}
