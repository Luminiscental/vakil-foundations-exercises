...
\begin{enumerate}[label=\textbf{5.5.\Alph*.}]
	...
	\item Since $\Supp(m)=V(\Ann(m))$, the associated primes of $M$ are those
	      primes in $A$ minimal over some $\Ann(m)$. Now in $S^{-1}M$ we have
	      $\Ann(m/s)=S^{-1}\Ann(m)$, so the associated primes of $S^{-1}M$ are
	      those primes in $S^{-1}A$ minimal over some $S^{-1}\Ann(m)$. These
	      correspond to the primes in $A$ minimal over $\Ann(m)$ that don't meet
	      $S$, since a prime minimal among those not meeting $S$ is minimal
	      generally.
